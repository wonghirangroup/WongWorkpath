...
| `#767676` | Tertiary muted text (role line, empty-value hints) — darkened from the old `#A0A0A0`/`#B0B0B0` (2.4:1) so it reaches 4.5:1 on white (WCAG AA) |
| `#B0B0B0` | Input placeholder text only (placeholders keep the lighter tone; real text never uses it) |
...
- **Confirmation popup** (`components/ConfirmDialog.tsx`, opened with `const confirm = useConfirm()` from `context/ConfirmContext.tsx` — `if (!(await confirm({ title, message, tone }))) return;`): every action that **saves an edit to existing data or deletes/removes something** asks first, every time, in every module. The popup is a `max-w-sm` `rounded-2xl` card at `z-[70]` (above the modal it usually opens from) with a round icon, bold title, muted `text-xs` message naming the item, an outline "ยกเลิก" and a full-width action button. `tone: 'default'` = saving an edit (orange `#FF6537`, pencil icon, focus starts on the action button); `tone: 'danger'` = a delete (red, trash icon, focus starts on **ยกเลิก** so a stray Enter can't destroy anything). Escape / backdrop click / X all cancel, and Escape is caught in the capture phase so it closes only the popup, never the modal underneath. Put the `await confirm()` in the form's `handleSubmit` after validation and before the request starts. **Not** confirmed (by decision): creating new records, workflow status buttons (ส่งงาน / ตรวจงาน "ผ่าน"), notification switches, and other UI-only preferences. Flows that already had their own real modal (delete project/document/employee/org unit, cancel meeting, credential delete with typed name) keep it instead of stacking a second popup.
- **Wide form modals**: a form with many fields uses a wider box and a two-column grid instead of `max-w-md` with one long scrolling column — Employee Management's "เพิ่มพนักงานใหม่" and the employee detail/edit modal are both `max-w-3xl` (`p-6`, `max-h-[90vh] overflow-y-auto`, fields in `grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3`, full-width rows like จำกัดสิทธิเมนู/รูปโปรไฟล์ via `sm:col-span-2`); the project modals sit between (`max-w-lg` create/edit project, `max-w-xl` schedule meeting, `max-w-2xl` add task).
...
- **Org chart edit-mode icons**: `Pencil` (rename, small circular button top-left of a box), `Trash2` (delete, top-right — opens a confirmation modal), `Plus` (add division/section, dashed-border button), `ZoomIn/ZoomOut/Maximize2` (the pan/zoom canvas's floating controls, bottom-right).
...
- **Every edit-save and every delete asks first** — see "Confirmation popup" under Shared Components. The old inline two-step "ลบ?" arm/confirm (org chart deletes, project task/subtask deletes) no longer exists; both now use a real popup.
...
Open to every account type and never restrictable per-account (like dashboard; excluded from `RESTRICTABLE_NAV_ITEMS`). Reached from the Header user menu ("การตั้งค่า") and its own Sidebar item (lucide `Settings` icon — no PNG set exists).

**Two-pane layout** (`SettingsView.tsx`): a **Menu Zone** on the left (`lg:w-72` white card, `rounded-2xl`) and a **Work Space** on the right that takes all remaining width (same card treatment). Below `lg` the menu drops the card and becomes a horizontally scrolling row of bordered pills stacked above the workspace.
- Menu entries are grouped under the spec's two zone labels (`text-[11px] font-bold text-[#767676]`): **บัญชีของฉัน** → โปรไฟล์, เปลี่ยนรหัสผ่าน; **การแจ้งเตือน** → เปิด/ปิดการแจ้งเตือน. Entry = `h-11 rounded-xl px-3 text-sm font-semibold` with a lucide icon; colored exactly like every other tab/nav in the app — active is the solid brand orange `bg-[#FF6537] text-white` (same as the Sidebar pill and the segmented tabs), inactive `text-[#6F6F6F] hover:text-[#272220] hover:bg-slate-50`. A small dot on โปรไฟล์ = a name-change request is pending (amber, white when the entry is active); a "ปิด N" chip on the notifications entry = N categories switched off (`bg-[#F4F4F5] text-[#6F6F6F]`, `bg-white/25 text-white` when active).
- The workspace opens with a `text-[15px] font-bold` title + `text-xs text-[#6F6F6F]` description for the selected entry, then that entry's form. All drafts live in one component, so switching entries never loses unsaved input.
- **โปรไฟล์** — read-only by default (inputs disabled `#F9F9F9`, no avatar buttons) with an outlined orange "แก้ไข" button top-right of the pane header (same style as the employee detail modal's "แก้ไขข้อมูล"); it unlocks the form and swaps in "ยกเลิก" (discards the draft) + "บันทึก", and saving asks via the confirmation popup. Editing shows avatar + "เปลี่ยนรูป", then a 2-column grid of ชื่อ-นามสกุล / ชื่อเล่น / เบอร์โทร / อีเมล (inputs `h-11 rounded-xl border-[#E5E5E5]`, focus `#FF6537`, disabled `#F9F9F9`). A plain employee changing ชื่อ-นามสกุล or ชื่อเล่น gets a required "เหตุผล" textarea and the button becomes "บันทึกและส่งคำขอ"; while a request is pending those two inputs are disabled and an amber banner (`bg-amber-50 border-amber-200`, `Clock` icon) lists the requested values + reason. Admin/Super Admin/ผู้บริหาร save directly. Admins decide requests from a "คำขอเปลี่ยนชื่อรออนุมัติ" amber panel (reusing `PendingRequestCard`) at the top of Employee Management → รายชื่อพนักงาน.
- **เปลี่ยนรหัสผ่าน** — new + confirm with eye toggles; the once-per-day limit is in the description and a 429 shows inline.
- **การแจ้งเตือน** — one switch per category, not a master switch: a hairline-divided list (`divide-y divide-[#EDEEEF]`), each row `px-6 py-4` with a bold label + muted description on the left and a `ToggleSwitch` (`components/ToggleSwitch.tsx`: `h-6 w-11 rounded-full`, `#FF6537` on / `slate-300` off, `role="switch"`) on the right. The six categories (`lib/notificationCategories.ts`): การมอบหมาย, การตรวจงาน, งานติดปัญหา, กำหนดส่งงาน, นัดประชุม, คำขออนุมัติ — every `pushNotification` call in `AppDataContext.tsx` is tagged with one (legacy handover notifications carry none and are never filtered). A muted category is hidden by the server (`GET /api/notifications`) but its rows keep accumulating and reappear when switched back on. A switch moves the instant it's clicked (optimistic) and saves in the background — never dim/disable the switches while saving (that read as a flicker); saves go out one at a time and rapid clicks collapse into the latest list so nothing is overwritten. No confirmation popup here (see Confirmation popup above).
- Every form's primary submit is right-aligned, `#FF6537` / disabled `#F68C6C`, with a green `Check` + text confirming a save beside it.

## Responsive / mobile (below `lg`, 1024px)

Checked at 390×844, 360×740 and 768×1024 (no page may scroll sideways) and against desktop 1024–1536 (unchanged).

- **Shell**: the Sidebar becomes a drawer opened from the Header's hamburger (`#mobile-menu-toggle`). z-index ladder, low → high: table sticky headers `z-20` → every page's sticky toolbar `z-30` → app Header `z-40` → **mobile drawer `z-[45]`** → modals `z-50` → `ConfirmDialog` `z-[70]`. The drawer must stay above the sticky toolbars (at `z-30` the Dashboard's filter bar painted over the menu) and deliberately covers the Header, since it has its own logo + close row. Drawer items are touch-sized (`py-3 text-sm`, 18px icons, close button has a 40px hit area).
- **Toolbars** (Dashboard, Doc Vault, Credential Vault, Org chart, Project detail header): wrap only below `lg` (`flex-wrap lg:flex-nowrap`) — from `lg` up they keep their original single row. The primary action drops to its own line and goes full width (`max-sm:w-full`) so it is never cut off at the right edge.
- **Wide tables** scroll sideways inside their card (`overflow-x-auto`) and get a `min-w-*` so columns keep their natural width instead of being squeezed (the project "งาน" table's title column used to collapse to 1–2 characters per line).
- **Card view by default on phones** for the project board and Credential Vault (`isPhoneViewport()` in `lib/viewport.ts`, only the initial state — the toggle still works); Doc Vault and Employee Management already defaulted to cards.
- **Calendar**: below `lg` the page sizes to its content instead of `h-full` (which squeezed both cards into one viewport height and let the filter card's department dropdown spill over the calendar), the month grid gets an explicit height (`23rem`, Gantt `26rem`), and the calendar comes first with the filters after it (`max-lg:order-last`). From `lg` up: filters left, calendar right, both filling to the Sidebar's bottom edge as before.
- Modals were already phone-ready (`mx-4`, `max-h-[85–90vh]`, scrolling body, sticky footer) — keep that shape for new ones.
